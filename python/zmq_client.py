...
class ZmqDriverClient(object):
    ...
    def _connect_driver(self):
        ...
        event_host_string = 'tcp://%s:%i' % (self.host, self.event_port)

        self.zmq_cmd_socket.connect(cmd_host_string)
        log.info('Driver client cmd socket connected to %s.', cmd_host_string)

        self.zmq_evt_socket.connect(event_host_string)
        self.zmq_evt_socket.setsockopt(zmq.SUBSCRIBE, '')
        log.info('Driver client event thread connected to %s.', event_host_string)
    ...
    def start_messaging(self, callback=None):
        ...
        def event_loop():
            while not self._stopped:
                try:
                    evt = self.zmq_evt_socket.recv_json(flags=zmq.NOBLOCK)
        ...
        self.event_thread = spawn(event_loop)
    ...
    def _command(self, cmd, *args, **kwargs):
        ...
        timeout = kwargs.pop('driver_timeout', 600)
